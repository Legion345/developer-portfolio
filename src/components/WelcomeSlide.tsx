 ...
 */
const SEEN_KEY = "welcomeSlideSeen";

export function WelcomeSlide() {
  // Read synchronously on first render so the splash never flashes for
  // visitors who have already seen it this session (e.g. navigating back
  // from a project page). sessionStorage clears when the tab closes.
  ...
  useEffect(() => {
    // Skip the whole sequence (and scroll lock) if already shown this session.
    if (isHidden) return;

    // Remember that the splash has played so back-navigation doesn't replay it.
    ...
    >
      ...
      >
        ...
        <p className="text-base leading-relaxed mb-6">
          A developer built this instead of touching grass. We&apos;re just
          tidying up, and then we&apos;ll pretend this was intentional the whole
          time.
        </p>
        <p className="text-base mb-8">{progress}% complete</p>
        <p className="text-sm leading-relaxed mb-8">
          Stack trace successfully ignored:
          <br />
